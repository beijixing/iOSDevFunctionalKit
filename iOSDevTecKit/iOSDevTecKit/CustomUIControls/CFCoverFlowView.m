...
- (void)setPageItemsWithImageNames:(NSInteger)itemCnt{
    // page items count > 3
    assert(itemCnt != 0 && itemCnt> 3);

    _pageItemSpace = _pageItemWidth -  _pageItemCoverWidth;
    _pageItemCount = itemCnt;
    for (NSInteger i = 0; i < _pageItemCount; i++) {
        UIView *view = [[UIView alloc] init];
//        view.backgroundColor = [UIColor darkGrayColor];
//        view.center = CGPointMake(_width / 2.0, _height/2.0);
        view.bounds = CGRectMake(0, 0, _pageItemWidth, _pageItemHeight);
        view.contentMode = UIViewContentModeScaleAspectFill;
        view.userInteractionEnabled = YES;
        view.layer.cornerRadius = _pageItemCornerRadius;
        view.layer.masksToBounds = YES;
        view.layer.shouldRasterize = YES;
        view.layer.rasterizationScale = [UIScreen mainScreen].scale;
        view.tag = i;
        view.layer.position = CGPointMake(_width / 2.0, _height/2.0);
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(coverFlowView:addDataView:atIndex:)]) {
            [self.delegate coverFlowView:self addDataView:view atIndex:i];
        }
...
- (void)scrollTopageWithIndex:(NSInteger)index {
    [self transformWithDistance:-index];
    [self animationWithDistance:-_pageItemSpace + index duration:0];
...
- (CATransform3D)transformForPosition:(CGFloat)position {
    CATransform3D translationTransform = CATransform3DMakeTranslation(position, 0.0, fabs(position / _pageItemSpace) * -300.0);
    CATransform3D transform = CATransform3DRotate(translationTransform, -(M_PI * (position / (_pageItemSpace / 1.0)) / 180.0), 0, 1, 0);
    return transform;
}
